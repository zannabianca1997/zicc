//! Streams based on [`std::io`] streams
...
impl<R: io::Read> Iterator for IoReader<R> {
...
mod tests {
    ...
    use super::{Format, IoReader, IoWriter};

    fn collect_ints(input: &[u8]) -> Vec<Value> {
        IoReader::new(Format::Ints, input)
            .map(|r| r.unwrap())
            .collect()
    }
    ...
    fn write_all(format: Format, values: &[Value]) -> Result<Vec<u8>, super::Error> {
        let mut buf = Vec::new();
        let mut w = IoWriter::new(format, &mut buf);
        for v in values {
            w.write(v.clone())?;
        }
        Ok(buf)
    ...
    fn reader_should_return_none_on_empty_int_stream() {
        let mut r = IoReader::new(Format::Ints, b"".as_ref());
    ...
    fn reader_should_read_bytes() {
        ...
        assert_eq!(
            IoReader::new(Format::Bytes, bytes)
                .map(|r| r.unwrap())
                .collect::<Vec<_>>(),
            nums(&[0, 255, 42])
        );
    ...
    fn reader_should_read_unicode_codepoints() {
        ...
        let got: Vec<Value> = IoReader::new(Format::Unicode, input)
            .map(|r| r.unwrap())
            .collect();
    ...
    fn reader_should_error_on_invalid_utf8() {
        let mut r = IoReader::new(Format::Unicode, [0xFFu8].as_ref());
    ...
    fn reader_should_error_on_truncated_utf8() {
        let mut r = IoReader::new(Format::Unicode, [0xE2u8, 0x82].as_ref());
        assert!(matches!(r.read(), Err(super::Error::InvalidUtf8)));
    }
    ...
    fn reader_should_iterate() {
        let values: Vec<Value> = IoReader::new(Format::Ints, b"10 20 30".as_ref())
            .map(|r| r.unwrap())
            .collect();
    ...
    fn writer_then_reader_should_roundtrip_ints() {
        ...
        let recovered: Vec<Value> = IoReader::new(Format::Ints, bytes.as_slice())
            .map(|r| r.unwrap())
            .collect();
