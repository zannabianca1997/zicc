...
pub struct ICMachineData {
    ...
    inp_queue: VecDeque<ICMachineIntType>,
    out_queue: VecDeque<ICMachineIntType>,
}

type ICParam = (ICParamMode, ICMachineIntType);

impl ICMachineData {
    /// Read from the machine memory
    fn read_mem(&self, pos: usize) -> ICMachineIntType {
    ...
    fn write_mem(&mut self, pos: usize, value: ICMachineIntType) {
        if pos >= self.memory.len() {
    ...
    /// Read from a param
    fn read_param(&self, param: ICParam) -> Result<ICMachineIntType, ICRuntimeErr> {
        match param.0 {
            ...
            ICParamMode::Relative => {
                if (param.1 + self.rb as ICMachineIntType) >= 0 {
                    Ok(self.read_mem((param.1 + self.rb as ICMachineIntType) as usize))
                } else {
                    Err(ICRuntimeErr::ReadingFromNegative)
                }
            }
    ...
    /// Write to a param
    fn write_param(&mut self, param: ICParam, value: ICMachineIntType) -> Result<(), ICRuntimeErr> {
        match param.0 {
            ICParamMode::Position => {
                if param.1 >= 0 {
            ...
            ICParamMode::Relative => {
                if (param.1 + self.rb as ICMachineIntType) >= 0 {
                    self.write_mem((param.1 + self.rb as ICMachineIntType) as usize, value);
                    Ok(())
                } else {
                    Err(ICRuntimeErr::WritingToNegative)
                }
    ...
    macro_rules! ic_tests {
        ...
        $(
            mod $name {
                ...
                const PROGRAM: &[ICMachineIntType] = &[$($progv ,)*];
                $(
                    #[test]
                    ...
                    fn $io_name() {
                        ...
                        // unloading output
                        let mut output: Vec<ICMachineIntType> = vec![];
                        loop {
