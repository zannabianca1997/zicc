...
pub fn deduplicate<'d>(
    ...
    let sequential_ids = uniques.iter().enumerate().map(|(seq, id)| (*id, seq + 3)).collect::<BTreeMap<_,_>>();
    ...
    final_types[INT_ID.into_id().0] = Some(TypeEntry { typ: Type::Data(TypeData::Int(TypeInt)), name: None, size: Some(1) });
    final_types[UNKNOW_ID.0] = Some(TypeEntry { typ: Type::Unknow(TypeUnknow), name: None, size: None });
    final_types[UNIT_ID.into_id().0] = Some(TypeEntry { typ: Type::Data(TypeData::Composite(TypeComposite{name:None,fields:BTreeMap::new(), size:0})), name: None, size: Some(0) });
    // adding the other type
    for unique_id in uniques {
        let ty =  types[unique_id].take().expect("Uniques should guarantee no double take");
        use super::TypeEntry::*;
        let entry = match ty {
            Array { def:_, element, lenght, size } => TypeEntry { 
                typ: Type::Data(
                    TypeData::Array(
                        TypeArray{
            ...
            Composite { def:_, name, fields, size } => TypeEntry { 
                typ: Type::Data(
                    TypeData::Composite(
                        TypeComposite { 
                            name, 
                            fields: fields.into_iter().map(|(name, (offset, r))| (name, (offset, reference_to_type_id_data(r)))).collect(), 
                            size
                        }
                    )
                ), 
            ...
            Pointer { def:_, kind, pointee } => TypeEntry { 
                typ: Type::Data(
                    TypeData::Pointer(
                        TypePointer {
                            kind,
            ...
            Fn { def:_, inputs, output } => TypeEntry { 
                typ: Type::Fn(
                    TypeFn{
    ...
    let types = final_types.into_iter().map(|o| Box::new(o.expect("All types should have been added"))).collect();

    // Finally, we map the names to the new ids
    let names = names.into_iter().map(|(name, id)| (name, reference_to_type_id(id))).collect();
...
fn merge_success(
    ...
    let mut check_referred = move |into: ReferredType, from: ReferredType| -> bool {
        match (into, from) {
            ...
            (ReferredType::Base(_), ReferredType::Id(_)) |(ReferredType::Id(_), ReferredType::Base(_)) => false,
            (ReferredType::Id(a), ReferredType::Id(b))  if id_map[a] == id_map[b]=> true,
            (ReferredType::Id(a), ReferredType::Id(b)) => merge_success(id_map[a], id_map[b], id_map, unduplicated, types, uniques)
        }
    };
    ...
    match (&types[into], &types[from]) {
        ...
        | (Entry::Pointer { pointee: a,.. }, Entry::Pointer { pointee:b,.. }) => check_referred(*a,*b),
        // Composites must merge their field type by name
        (Entry::Composite { fields: a,.. }, Entry::Composite { fields:b,.. }) => a.iter().all(|(name, (_, a_ty))| {
            check_referred(*a_ty, b[name].1)
        }),
        // Functions must match inputs and output
        (Entry::Fn {  inputs: a_inputs, output: a_output,.. }, Entry::Fn { inputs: b_inputs, output: b_output,.. }) => {
            a_inputs
            .iter()
            .chain(Some(a_output))
            .zip(
...
fn compatible(into: &Entry, from: &Entry) -> bool {
    match (into, from) {
        ...
        (Entry::Array {  lenght: length_a, size: size_a, ..}, Entry::Array { lenght: length_b, size: size_b, ..}) => length_a == length_b && size_a == size_b,
        // Pointers must match in kind
        (Entry::Pointer { kind: kind_a,.. }, Entry::Pointer { kind:kind_b,.. }) => kind_a == kind_b,
        // Composites must have the same name and size, and the named fields must be present in both and be at the same offset
        (Entry::Composite { name: name_a, fields:fields_a , size: size_a, .. }, 
            Entry::Composite { name: name_b, fields:fields_b , size: size_b, .. }) =>  {
            name_a == name_b && size_a == size_b && 
                fields_a.keys().chain(fields_b.keys()).unique().all(|field| {
                    if let (Some((off_a, _)), Some((off_b,_))) = (fields_a.get(field), fields_b.get(field)) {
                        off_a == off_b
                    }else {
                        false
                    }
        ...
        (Entry::Fn {  inputs: a_inputs, .. }, Entry::Fn { inputs: b_inputs, .. }) => {
